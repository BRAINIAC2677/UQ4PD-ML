...
class MCDropout(nn.Module):
    ...
    def forward(
        ...
        Returns:
            Tensor: output tensor of shape (B, ...) during evaluation.
        """
        if self.training:
            return self.core_model(x)
        ...
        if self.on_batch:
            ...
            predictions = self.core_model(x)
            # Reshape and compute the mean over estimators
            predictions = predictions.view(self.num_estimators, -1, *predictions.shape[1:])
            return predictions.mean(dim=0)

        # Else, for loop
        predictions = torch.stack([self.core_model(x) for _ in range(self.num_estimators)], dim=0)
        return predictions.mean(dim=0)
